%% Script to convert an rnxbinaries file to Rinex format for further analysis:
% Inputs:
%   - filename set in file settings (name of the binary file being analyzed)
% Ouputs: 
%   - generates a text file that is saved in the following format
%       MM_DD_YYYY_rinex.txt
%   - saves the file in RinexOutputFiles directory

clc;
clear all;
close all;

NAME_OF_INPUT_FILE = 'rnxBinaries_0_0.bin';


%% File settings
rinexInputStr = '../../output/';
rinexOutputStr = '../../output/RINEX_conversion/';
fileName    = [rinexInputStr NAME_OF_INPUT_FILE];    % file name 
myfile      = dir(fileName);        % file info
if isempty(myfile)
    error(['Wrong Directory: Current Directory -' rinexInputStr ' does not contain the file.']);
end
...
%% -------------------------Open New File--------------------------------
dataIndex = ~isnan(pos_enu(:,1));
weekNumber = pos_enu(dataIndex,1);
weekNumber = weekNumber(1);
TOW = pos_enu(dataIndex,2);
TOW = int64(TOW(1));

fid = fopen([rinexOutputStr 'rinex' '_' num2str(weekNumber) '_' num2str(TOW) '.txt'],'w');  
%% ---------------------------Header Data--------------------------------
...
disp(['Finished Writing Rinex File: ' 'rinex' '_' num2str(weekNumber) '_' num2str(TOW) '.txt']);